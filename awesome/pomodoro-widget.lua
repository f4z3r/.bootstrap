...
function Pomodoro:new(args)
  ...
  obj.current_color = obj.color_pausing
  ...
  obj.timeout = 5
  obj.timer = timer { timeout = obj.timeout }
...
function Pomodoro:update()
  ...
  local prefix = ""
  if time_left <= 0 and time_left > -self.timeout then
    local notif = self.notif_go_back
    local notif_color = self.color_working
    self.finished = true
    if self.working then
      self:add()
      notif = self.notif_done
      notif_color = self.color_pausing
    end
    self:notify(notif, notif_color)
  ...
  local formated_time = os.date("%M:%S", time_left)
  self.widget:set_markup(self.format(prefix .. formated_time, self.current_color, self.current_symbol, self.completed))
end


function Pomodoro:notify(t, c)
  naughty.notify {
